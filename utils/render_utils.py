...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
    """Creates a video from rendered RGB images using OpenCV."""
    import cv2
    ...
    file0 = os.path.join(input_dir, f'{idx_to_str(0)}.png')
    
    if not os.path.exists(file0):
        raise ValueError(f'Missing initial frame: {file0}')
    ...
    try:
        for idx in tqdm(range(num_frames)):
            img_file = os.path.join(input_dir, f'{idx_to_str(idx)}.png')
            if not os.path.exists(img_file):
                raise ValueError(f'Missing frame: {img_file}')
